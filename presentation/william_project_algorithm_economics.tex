...
\begin{frame}
	\frametitle{What the Guarantees Rest On: Five Assumptions}
	\small
	The mechanism's truthfulness and aggregation guarantees are \emph{theoretical}: they rely on five assumptions about agents and their signals.

	\medskip
	\begin{itemize}
		\setlength{\itemsep}{0.35em}
		\item[\textbf{A1}] \textbf{Rationality \& risk-neutrality.} Agents are fully rational, risk-neutral Bayesians --- and this is common knowledge.
		\item[\textbf{A2}] \textbf{Common prior.} Signals $X^{(t)}$ and outcome $Y$ are drawn from a common prior that is common knowledge.
		\item[\textbf{A3}] \textbf{Stochastic relevance.} Each signal induces a \emph{unique} posterior, so agents can invert others' reports (``distinct signals'').
		\item[\textbf{A4}] \textbf{Conditional independence.} Signals are independent given $Y$ --- i.e.\ informational \emph{substitutes}.
		\item[\textbf{A5}] \textbf{$(\delta,\eta)$-informative signals.} A floor on how informative each signal is ($\delta$) and a floor on signal probabilities ($\eta$).
	\end{itemize}

	\medskip
	\centering
	\alert{Our two experiments probe what happens when these hold --- and when they don't.}
\end{frame}

\begin{frame}
	\frametitle{What the Paper Proves: Three Guarantees}
	\small
	All three say the same thing from different angles: \alert{a well-informed reference makes honesty optimal.}

	\smallskip
	\begin{block}{Thm 1 --- A report's influence is bounded}
		If the reference observes $k$ informational substitutes that agent $t$ cannot, then $t$'s influence on the reference's posterior --- the adjustment $\Delta$ --- shrinks: $|\Delta|\le\varepsilon$ once $k \gtrsim \frac{1}{-\log(1-\delta)}\log\frac{1}{\varepsilon}$. \emph{Enough substitutes $\Rightarrow$ reference $\approx$ ground truth.}
	\end{block}

	\begin{block}{Thm 2 --- Truthfulness becomes strictly optimal}
		Paid by cross-entropy scoring against a truthful reference, agent $t$ \emph{strictly} maximizes expected payoff by reporting truthfully once the reference has $\ge k$ signals (an explicit threshold in $\delta,\eta,\tau$ and the prior).
	\end{block}

	\begin{block}{Thm 3 --- The gain from lying vanishes}
		The gain from \emph{any} deviation is bounded by $D_\eta(\Delta,\mathbf{y})$; since $\Delta\to0$ as $k$ grows (Thm 1), this bound $\to0$ --- and crucially, no $\tau$ is needed.
	\end{block}
\end{frame}
...
\begin{frame}
	\frametitle{RQ1: Testing the Theory on Its Own Terms}
	\small
	The simulator is built so that \alert{A1--A4 hold by construction}:
	\begin{itemize}
		\setlength{\itemsep}{0.3em}
		\item[\textbf{A1}] agents are Bayesian by construction; the focal agent deviates \emph{within} that rational frame (via $\gamma$).
		\item[\textbf{A2}] a single common prior $\pi$ generates every signal.
		\item[\textbf{A3--A4}] signals are distinct, conditionally independent draws given $Y$.
	\end{itemize}

	\begin{block}{RQ1 stress-tests A5 (informativeness)}
		The one assumption we \emph{vary} is how informed the reference is. Sweeping $k$ (the reference's informational substitutes) asks: how informed must the reference be before truthful reporting is the best response? (Thm.~3: deviation gain $\to 0$ as $k$ grows.)
	\end{block}

	\centering
	\alert{Clean world $\Rightarrow$ isolates the mechanism's core incentive claim.}
\end{frame}
...
\begin{frame}
	\frametitle{RQ2: What Happens When the Assumptions Break}
	\small
	Real Polymarket traders satisfy almost none of the paper's assumptions:
	\begin{itemize}
		\setlength{\itemsep}{0.3em}
		\item[\textbf{A1}] \alert{violated} --- real traders are not risk-neutral, fully rational Bayesians.
		\item[\textbf{A2}] \alert{violated / unverifiable} --- no shared common prior or common knowledge.
		\item[\textbf{A4}] \alert{violated} --- price moves are serially correlated (herding, momentum, shared news), not conditionally independent given $Y$.
		\item[\textbf{A3, A5}] unverifiable / not enforced on observed price paths.
	\end{itemize}

	\begin{block}{So the theory gives \emph{no} guarantee here}
		RQ2 is therefore a \alert{robustness test}: does the self-resolving payout still track the verifiable one when the assumptions it relied on are broken?
	\end{block}
\end{frame}
...
\begin{frame}
	\frametitle{RQ2: Agreement vs.\ \emph{Relative} Time to Close}
	...
	\begin{figure}
		\includegraphics[width=0.92\linewidth]{Images/emp_fraction_sweep.png}
	\end{figure}

	{\footnotesize Agreement rises as the reference approaches close: Pearson $0.94\!\to\!1.00$, Spearman $0.67\!\to\!1.00$, payout ratio $0.48\!\to\!0.94$. Even at a genuinely early reference the payout \emph{levels} still track (Pearson $\approx0.94$); the \emph{ranking} and the designer's total cost are what the leakage channel inflates near resolution.}
\end{frame}
...
\begin{frame}
	\frametitle{Results RQ2}
	\begin{itemize}
		\item Payouts track almost perfectly.
		\item Ranking degrades as the reference moves earlier.
		\item The gap is likely due to outcome leakage.
		\item The mechanism does seem to be able to track the correct outcome, especially at late references.
	\end{itemize}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		\item Can the mechanism be adjusted so the ranking (who deserves the most reward) better matches who actually moved price toward truth, without needing the outcome?
		\item Can we adjust the mechanism to non binary predictions?
		\end{itemize}
\end{frame}
